...
- 本项目为**完全脱敏开源版**，不包含任何历史开发人员的测试账号、手机号码及缓存的 Cookie / 会话凭据。
- 所有的敏感文件（如包含本地登录信息的 `config/login_info.json`）均已加入 `.gitignore` 保护，防止意外提交。
- 本地接口在接收到登录信息后会将其保存在您本地挂载的 `config/login_info.json` 中，请确保您的部署服务器（如 VPS）端口安全，不要向公网暴露未经防火墙保护的 `10000` 端口。
